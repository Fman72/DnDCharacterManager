...
// Quick and dirty JS to turn spells from GitHub in Django Fixtures.
...
const getUniqueKeys = (spells) => {
  keys = {}

  spells.forEach((spell, index) => {
    Object.keys(spell).forEach((value, index) => {
      if (!keys[value]) {
        keys[value] = 1;
      }
    });
...
const spellTransform = (spellData, index) => {
  data = {
    model: 'api.ability',
    pk: index,
    fields: {}
  }
  Object.keys(spellData).forEach((value, index) => {
      if (value === 'desc') {
          data.fields.description = spellData[value];
      }
      else if (value === 'ritual') {
          data.fields.ritual = spellData['ritual'] === 'yes';
      }
      else if (value === 'concentration') {
          data.fields.concentration = spellData['concentration'] === 'yes';
      }
      else if (value === 'casting_time') {
          data.fields.castingTime = spellData['casting_time'];
      }
      else if (value === 'higher_level') {
        data.fields.higherLevel = spellData['higher_level'];
    }
      else if (value === 'class') {
          data.fields.spellClass = spellData['class'];
      }
      else if (value === 'level') {
          const level = spellData['level'][0];
          data.fields.level = level === 'C' ? 0 : level;
      }
      else if (value === 'page') {
        
      }
      else {
          data.fields[value] = spellData[value];
      }
  });
  return data;
}
...
acIndex = 1
const abilityClassTransform = (spellData) => {
  res = []
  spellData.forEach((spell, index) => {
      classes = spell['class'].split(',')
      classes.forEach((className) => {
        data = {
          model: 'api.abilityClass',
          pk: acIndex++,
          fields: {}
        };
        className = className.trim();
        classId = classIdMap[className];
        data.fields['characterClass'] = classId;
        data.fields['ability'] = index + 1;
        if (classId) {
          res.push(data);
        }
      });
...
spells.forEach((spell, index) => {
  res.push(spellTransform(spell, index + 1))
});

//console.log(getUniqueKeys(spells))

//console.log(JSON.stringify(res));

console.log(JSON.stringify(abilityClassTransform(spells)))